...
background = cv2.imread('/Users/anastasiaborisova/Desktop/bg_1.jpg')
background = cv2.resize(background, (width, height))

apple = cv2.imread('/Users/anastasiaborisova/Downloads/images-removebg-preview.png', cv2.IMREAD_UNCHANGED)
...
def draw_objects(field):
    ...
    for segment in snake[1:]:
        cv2.rectangle(field, segment, (segment[0] + size, segment[1] + size), (256, 0, 0), thickness=-1)
    ...
    for i in range(size):
        for j in range(size):
            if 0 <= y + i < height and 0 <= x + j < width:
                for c in range(3):
                    field[y + i, x + j, c] = alpha_mask[i, j] * bgr[i, j, c] + (1 - alpha_mask[i, j]) * field[y + i, x + j, c]
